...
% in ARC-paper an enumeration of use cases for caches
...
To uniquely identify objects each object has a handle, the \emph{ObjectKey}, which is an enumeration type that represents the state of the object at the last access, as shown in \ref{lst:ObjectKey enumeration}.
The \emph{G} type in \ref{lst:ObjectKey enumeration} stands for generation and is used by the upper layer to keep track of the age of an object.
...
\todo[inline]{find out what the use of "generation: G" is}
% \cite{wiedemann2018modern} generation is used by the upper layer to track age of an object try to verify this.

Thus, an object in DML (and Cache) can only be in unmodified, modified or write-back state.
...
%\todo[inline]{get syntax highlighting for Rust}
%\begin{lstlisting}[mathescape=true,caption=simplified Cache trait,label=lst:simplified Cache trait]
%$\textcolor{ovgu-blue}{trait}$ Cache {
%    $\textcolor{ovgu-blue}{fn}$ new
%    $\textcolor{ovgu-blue}{fn}$ contains_key
%    $\textcolor{ovgu-blue}{fn}$ get
%    $\textcolor{ovgu-blue}{fn}$ get_mut
%    $\textcolor{ovgu-blue}{fn}$ remove
%    $\textcolor{ovgu-blue}{fn}$ force_remove
%    $\textcolor{ovgu-blue}{fn}$ change_key
%    $\textcolor{ovgu-blue}{fn}$ force_change_key
%    $\textcolor{ovgu-blue}{fn}$ evict
%    $\textcolor{ovgu-blue}{fn}$ insert
%    $\textcolor{ovgu-blue}{fn}$ iter
%    $\textcolor{ovgu-blue}{fn}$ size
%    $\textcolor{ovgu-blue}{fn}$ capacity
%    $\textcolor{ovgu-blue}{fn}$ stats
%    $\textcolor{ovgu-blue}{fn}$ verify
%}
%\end{lstlisting}
...
%\begin{lstlisting}[mathescape=true,caption=simplified Cache trait,label=lst:simplified Cache trait]
%$\textcolor{ovgu-blue}{trait}$ Cache {
%	$\textcolor{ovgu-blue}{fn}$ new(capacity: usize) -> $\textcolor{ovgu-blue}{Self}$
%	
%    type Key: Eq + Hash;
%    type Value;
...
%    fn get_mut(&mut self, key: &Self::Key, count_miss: bool) -> Option<Self::ValueRef>;
