...
It can be used either directly by an application,
 in which case Haura runs in the process
 context of the application, as shown in  
\cref{fig:directly Haura}, or use JULEA as a wrapper for Haura, as shown in
 \cref{fig:wrapper Haura}.
...
\begin{figure}[H]
	...
	\begin{subfigure}{\textwidth}
		\includegraphics[scale=0.4]{haura_directly.jpg}
		\caption{Direct usage of haura.}
		\label{fig:directly Haura}
	\end{subfigure}
	
	\begin{subfigure}{\textwidth}
		\includegraphics[scale=0.4]{haura_wrapper.jpg}
		\caption{Using a wrapper.}
		\label{fig:wrapper Haura}
	\end{subfigure}
	\caption{This figure shows the 2 options to use Haura either directly or 	
			through a wrapper like JULEA.}
\end{figure}
...
The incomplete state, as seen in \cref{lst:ObjRef enumeration}, is an intermediate state during serialization of an object for writing on disk or deserialization of an object from disk to main memory. 

% talk with Johannes what he means with his annotation
%The incomplete state, as seen in \cref{lst:ObjRef enumeration}, can only be 
%reached when an object is deserialized from disk.
%If the DMU encounter an incomplete object during runtime, Haura panics and 
%crashes.
...
This may cause the cache size to temporarily exceed the specified cache capacity.

% changed due to suggestion from Johannes
...
Furthermore, we have added the \texttt{get\_mut}()-function, \cref{lst:simplified 
Cache trait} line 5, 
since the \texttt{get}()-function does not allow that the state of the cache 
changes, respectively atomics are used and thus a read-lock is sufficient.
However, for ML-CLOCK it is necessary to change the state of the cache, not just updating the metadata for an entry where atomics are sufficent.
...
Newly insert entries are placed behind the hand which represents the tail of the list, if we see the hand as the head of the list.
...
However, CLOCK-Pro, unlike LIRS, is adoptable as it does not require a predefined parameter to control the number of hot and cold entries.
...
For our implementation, we use a circular doubly linked list instead of circular singly linked list, like in CLOCK.
...
Since hands should not point to the same entry.
...
For each entry we need to store additional metadata. This includes whether it is a hot or cold entry, resident or non-resident in the cache, has it been 
referenced in the near past, and whether or not the entry is in the test phase.
...
Furthermore, $\textbf{Hand}_{cold}$ turns referenced cold entries in test phase to hot entries. 
A reference during test phase means that the Inter-Reference recency for this entry is low enough to be a categorized as an hot entry.

$\textbf{Hand}_{test}$ marks the last entry in test phase and ends the test phase for all resident cold entries when it moves over them. 
...
and $m_c$ is decreased by one, if a cold entry leaves its test phase without being accessed again.
...
hand gets stuck on this entry, since the entry is pinned and cannot be removed,
which causes an endless loop.
To prevent this, this entry is 
removed and temporally saved. So that the hands can work as intended.
Afterwards, the pinned entry is inserted back into the cache.
...
Both circular list have one hand, the \textbf{C-hand} for the clean-clock and 
the \textbf{D-hand} for the dirty-clock.
When the \textbf{D-hand} move to the next entry, it moves the next entry 
...
Like the hand in CLOCK, the \textbf{C-hand} and \textbf{D-hand} search for an entry with the reference bit set to 0.
...
When a valid eviction candidate was found,
...
A learning operation is triggered if the ghost queue exceeds it capacity and thus an ghost entry at the front leaves the ghost queue.

So there are three possibilities to trigger a learning operation:
\begin{enumerate}
	\setlength\itemsep{-0.8em}
	\item During a cache hit,	
	\item An entry for eviction is already in the ghost queue,
	\item A ghost entry leaves the ghost queue.
\end{enumerate}


The case that the entry for eviction is pinned can be handled the same way as in 
CLOCK and GCLOCK, 
by leaving the pinned entry in the clock and continuing the search at the next 
entry.
...
In contrast to more advanced machine learning algorithm, a single-layer perceptron is easy to implement, requires no additional libraries
...
As secondary storage, we used CephFS which is shared between all nodes.
The used Rust compiler was rustc 1.71.1 and
Fio 3.33 was used generate I/O workloads.
...
\todo[inline]{update ylimits for plots}
