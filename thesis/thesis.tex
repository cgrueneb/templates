...
An increasing problem is the growing gap between processor frequency and memory latency.
In order to counter this problem, different memory technologies are combined in a memory hierarchy.
Fast but expensive memory is used in combination with slow but cheaper memory to achieve 
the best possible balance between cost, latency, bandwidth and capacity.
In addition to the memory hierarchy, caching and hierarchical storage management, 
also known as tiered storage, are used to move data between hierarchy levels.

For this thesis, we implement and evaluate different cache replacement policies for a hierarchical storage 
stack based on $B^{\varepsilon}$-tree, a write-optimized variant of the B-tree, which internally uses a cache 
to store frequently used data.
We will run various benchmarks for this purpose and examine both single threaded and multi threaded workloads. 
Based on these benchmark results, we will show that the choice for the cache replacement policy depends on 
the underlying access distribution as well as on the number of threads using the cache.
Furthermore, we will make a recommendation which cache replacement policy should be used for certain workloads.
...
% below this check again with deepl reached dayly limit
...
What also stands out for this specific benchmark, with equal ratio of read and write operation \cref{fig:multi uniform rw_50to50}, 
is that for the uniform random distribution the hit rate increases with larger thread count.
...
This suggests that the combination of increased thread count and proportion of write operations 
...
Which could indicate that ML-CLOCK has problems to recognize the I/O access 
pattern for higher thread counts and file sizes.
Another possible cause could be how the ghost queue is managed.
Whether a learn operation is triggered depends on if the ghost queue has already reached maximum size and 
therefore entries have to be removed when a cache entry will be evicted from the cache which are then used for learning.
...
Since this is felt by the entries before the synchronization, a large quantity of learning operation 
are triggered at one blow until the size of the ghost queue is again smaller equal to the size of the cache.
...
since the performance of the implemented strategies was also inconsistent and there is no clear better strategy.
