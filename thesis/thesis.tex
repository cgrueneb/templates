...
\todo[inline]{need to rewrite, most of it moved to Background}
...
Since the 1980s, CPU performance and clock frequency has grown faster than DRAM latency and bandwidth, so that CPU performance is several orders of magnitude higher today.
One of the reasons for this is that in the past CPU and memory were optimized according to different goals.
CPUs were optimized to achieve higher clock speeds through techniques such as instruction pipelining, out-of-order execution, instruction-level parallelism, or branch prediction.
DRAM and hard disks, on the other hand, have been optimized primarily for higher memory capacity.
This leads to the "processor-memory performance gap", also known as the "memory wall" problem \cite{wulf1995hitting}, as shown in the figure.
...
To reduce this problem ,the memory is built up hierarchically, as shown in figure \ref{fig:memory hierarchy}.
The memory is ordered from fast, expensive with small capacity, the register for each CPU-core, to increasingly slower, cheaper with higher capacity at the bottom of the hierarchy, which are hard-drives in some cases even tape drives.
The goal of this approach is to achieve a high performance with low cost and high memory capacity.
...
Also FIFO better suited for cases where strict size limits must be maintained,since no metadata is used.
However, the serious disadvantage is that FIFO suffers from the Bélády's anomaly \cite{10.1145/363011.363155} , which means that a larger cache size can lead to a higher cache miss rate. LRU and LFU do not suffer from this anomaly.
...
The variable k can be choosen usually k=2 gives best performance.
...
% in "traffic analysis of a web proxy caching hierarchy" also zipf distribution mentioned
