...
(3) detection of access patterns (strategies (1)-(2) from \cite{10.1145/511399.511340})
and (4) using ml-techniques, which we will consider in more detail next.
...
An \emph{A1$_{in}$} FIFO queue for new entries, \emph{A1$_{out}$} for entries recently evicted, so not resident in memory only meta information and \emph{A$_m$} as the main buffer for frequently accessed entries.
The problem is again that the user have to set parameters for the \emph{A1$_{in}$}, \emph{A1$_{out}$} and \emph{A$_m$} and Thresholds when an entry will be promoted to \emph{A$_m$} and for switch from \emph{A1$_{in}$} to \emph{A1$_{out}$}.
