...
On deletion a "tombstone message" will be inserted into the tree and will be flushed down the tree to a leaf, like an insertion message.
...
The development of Haura started by comparing the $B^{\varepsilon}$ tree with the ZFS and ext4 file systems \cite{wiedemann2018modern}.
...
the $B^{\varepsilon}$ trees process. A message for a key-message pair can contain arbitrary data or apply arbitrary code on data.
...
which includes caching objects in memory, tracking modifications to objects and write back modified objects to storage devices.
...
All cache entries are stored in a priority queue, usually implemented as doubly linked list or a min-heap, sorted by their access frequency.
...
Also, the space overhead is still low with only 0.75 percent of the cache size \cite{megiddo2004outperforming}.
...
One example of this strategy is LIRS, which uses the Inter-Reference Recency to categorize the cache entries in hot and cold entries.
