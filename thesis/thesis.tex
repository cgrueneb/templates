...
However, CLOCK-Pro, unlike LIRS, is customizable because it does not require a predefined parameter to control the number of hot and cold entries.
...
For each entry we have to store additional metadata.
...
The $\textbf{Hand}_{cold}$ points to the last resident cold entry and is used to find the entry for eviction. Furthermore, $\textbf{Hand}_{cold}$ can turn referenced cold entries, resident or non-
resident, in test phase to hot entries. 
...
Always an unreferenced resident cold entry is evicted.
In this respect $\textbf{Hand}_{cold}$ operates like the hand in CLOCK.
This means that $\textbf{Hand}_{cold}$ goes over all entries and removes the reference for cold entries not in test phase until it finds an resident cold entry which is unreferenced and thus can be evicted.
...
In addition, passed non-resident cold entries are evicted.
The reason for this is that the inter-reference recency has become too high to justify converting these entries to hot entries if these entries have not already been converted to hot entries.
